...
enum FlybuyPhoneInputType { enable, disable, error }

class FlybuyPhoneInput extends StatefulWidget {
  ...
  @override
  State<FlybuyPhoneInput> createState() => _FlybuyPhoneInputState();
}

class _FlybuyPhoneInputState extends State<FlybuyPhoneInput> with Utility {
  ...
  FlybuyPhoneInputType type = FlybuyPhoneInputType.enable;
  ...
  void didChangeDependencies() {
    ...
    String initCodeCountry =
        get(widgetConfig.fields, ['initCodeCountry'], defaultCountryCode);

    List<Map<String, String>?> dataExclude =
        _getExclude(list_countries.countries, excludeCountry);

    List<Map<String, String>?> dataInclude = _getInclude(
        dataExclude, list_countries.countries, includeCountry, initCodeCountry);

    String initialCountryCode = _getInit(dataInclude, initCodeCountry);

    Map<String, String>? init = dataInclude.firstWhere(
        (item) => item!['code'] == initialCountryCode,
        orElse: () => null);

    Map<String, String>? selectedCountry = dataInclude.firstWhere(
        (item) =>
            item!['code']!.toUpperCase() == initialCountryCode.toUpperCase(),
        orElse: () => init);
  ...
  void initState() {
    ...
    if (!widget.enabled) {
      type = FlybuyPhoneInputType.disable;
    }
  ...
  Color colorLine(ThemeData theme) {
    if (_focusNode!.hasFocus &&
        (type != FlybuyPhoneInputType.error &&
            type != FlybuyPhoneInputType.disable)) {
      return theme.primaryColor;
    }
    switch (type) {
      case FlybuyPhoneInputType.disable:
        return theme.disabledColor.withOpacity(0.2);
      case FlybuyPhoneInputType.error:
  ...
  void onFieldSubmitted(String value) {
    FlybuyPhoneInputType newType = !widget.enabled
        ? FlybuyPhoneInputType.disable
        : type == FlybuyPhoneInputType.error
            ? FlybuyPhoneInputType.error
            : FlybuyPhoneInputType.enable;
    setState(() {
      type = newType;
    });
            ...
            if (value.startsWith("+") && value.length > 2 && value.length < 5) {
              setState(() {
                _selectedCountry = countries.firstWhere(
                    (item) => item!['dial_code']!.startsWith(value),
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              padding: const EdgeInsets.only(
                  left: itemPaddingMedium, right: itemPadding),
                ...
                children: <Widget>[
                  TextField(
                    decoration: InputDecoration(
                      ...
                      hintText:
                          widget.searchText ?? translate('label_search_phone'),
                      border: border,
...
List<Map<String, String>?> _getInclude(
  ...
  if (include.isNotEmpty) {
    List<Map<String, String>?> initCountries = allCountries
        .where((element) => dataInit.contains(element!['code']))
        .toList();

    List<Map<String, String>?> newIncludes = dataExclude
        .where((element) => include.contains(element!['code']))
        .toList();
...
List<Map<String, String>?> _getExclude(
  ...
  if (exclude.isNotEmpty) {
    return allCountries
        .where((element) => !exclude.contains(element!['code']))
        .toList();
  }
  return allCountries;
}
...
String _getInit(List<Map<String, String>?> allCountries, String value) {
  ...
  bool checkExist =
      allCountries.indexWhere((element) => element!['code'] == value) > -1;
  if (checkExist) {
    return value;
  }
