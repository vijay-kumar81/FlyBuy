...
class ButtonWidget extends StatelessWidget
  ...
  Widget build(BuildContext context) {
    ...
    double radius =
        ConvertData.stringToDouble(get(styles, ['radiusItem'], 8), 8);

    Color backgroundItem = ConvertData.fromRGBA(
        get(styles, ['backgroundItem', themeModeKey], {}), theme.primaryColor);
    Color borderColor = ConvertData.fromRGBA(
        get(styles, ['borderColorItem', themeModeKey], {}), Colors.transparent);
    double borderWidth =
        ConvertData.stringToDouble(get(styles, ['borderWidgetItem'], 0));

    bool autoIconItem = get(styles, ['autoIconItem'], false);
    double iconSize =
        ConvertData.stringToDouble(get(styles, ['iconSizeItem'], 14), 14);
    Color iconColor = ConvertData.fromRGBA(
        get(styles, ['iconColorItem', themeModeKey], {}), Colors.white);
    ...
    String textTitle =
        ConvertData.textFromConfigs(title, settingStore.languageKey);
    ...
    Widget iconWidget = FlybuyIconBuilder(
      data: icon,
      ...
      child: buildFullButton(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            onPressed: () =>
                typeAction != 'none' ? navigate(context, action) : null,
