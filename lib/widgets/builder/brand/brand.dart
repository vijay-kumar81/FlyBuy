...
import 'package:flybuy/widgets/flybuy_brand_item.dart';
...
class _BrandWidgetState extends State<BrandWidget>
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(
      builder: (_) {
        ...
        List<Brand> emptyBrands =
            List.generate(limit, (index) => Brand()).toList();
        bool isShimmer = brands.isEmpty && loading;

        return Container(
          margin: ConvertData.space(margin, 'margin'),
          decoration: decorationColorImage(color: background),
          height: layout == Strings.brandLayoutCarousel ||
                  layout == Strings.brandLayoutSlideshow
              ? height
              : null,
          child: LayoutBuilder(
  ...
  Widget buildLayout(
    ...
    switch (layout) {
      case Strings.brandLayoutCarousel:
        double widthView =
            ConvertData.stringToDouble(get(styles, ['width'], 300), 300);
  ...
  Widget buildItem({
    ...
    Color textColor = ConvertData.fromRGBA(
        get(styles, ['textItem', themeModeKey], {}),
        theme.textTheme.titleLarge?.color);
    EdgeInsetsDirectional padding = ConvertData.space(
        get(styles, ['paddingContent'], {}), 'paddingContent');
