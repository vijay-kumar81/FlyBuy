...
import 'package:flybuy/utils/video_shop_mapper.dart';
import 'package:flybuy/widgets/builder/product_video_shop/widgets/video_infomation.dart';
...
class _ProductVideoShopWidgetState extends State<ProductVideoShopWidget>
  ...
  _reloadProduct() {
    if (_productsStore!.products.isEmpty) {
      ...
      _productsStore!.getProducts(customQuery: {
        ...
        'page': 1,
        'exclude': [...exProducts.map((e) => (e.id ?? '').toString()).toList()]
            .join(','),
        'per_page': _limit,
      });
  ...
  void didChangeDependencies() {
    ...
    if (_appStore.getStoreByKey(currentVideoStoreKey) != null) {
      _currentVideoPosition =
          _appStore.getStoreByKey(currentVideoStoreKey).page;
    }
  ...
  void dispose() {
    ...
    if (_watchedVideo.isNotEmpty) {
      ...
      _appStore.addStore(
        CurrentVideoStore(
            key: currentVideoStoreKey, page: _currentVideoPosition),
      );
    }
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    AlignmentDirectional actionsAlignment = ConvertData.toAlignmentDirectional(
        get(configs.styles, ['actionsAlignment'], 'bottom-end'));
    ...
    return Observer(builder: (_) {
      ...
      return Container(
          ...
          child: VideoShopFlutter(
                ...
                ? (video) {
                    return AddCartVideoShop(
                      product: products.firstWhere((element) =>
                          video?.id != null && element.id == video?.id),
            ...
            informationPadding: const EdgeInsetsDirectional.only(
                bottom: 120, start: 16, end: 96),
