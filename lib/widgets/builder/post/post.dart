...
import 'package:flybuy/widgets/builder/post/layout/layout_big_first.dart';
...
class _PostWidgetState extends State<PostWidget>
    with Utility, PostMixin, ContainerMixin {
  ...
  void didChangeDependencies() {
    ...
    // Gen key for store
    List<PostTag> tagsObj = tags
        .map((t) => PostTag(id: ConvertData.stringToInt(t['key'])))
        .toList();
    List<PostCategory> catsObj = categories
        .map((t) => PostCategory(id: ConvertData.stringToInt(t['key'])))
        .toList();
    List<Post> includePostObj = includePost
        .map((t) => Post(id: ConvertData.stringToInt(t['key'])))
        .toList();
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (_) {
        ...
        double? height = ConvertData.stringToDoubleCanBeNull(
            get(configs.styles, ['height'], null));

        int perPage =
            ConvertData.stringToInt(get(configs.fields, ['perPage'], 10));
        List<Post> emptyPosts =
            List.generate(perPage, (index) => Post()).toList();

        bool isShimmer = posts.isEmpty && loading;

        return Container(
          margin: ConvertData.space(margin, 'margin'),
          height: layout == Strings.postCategoryLayoutCarousel ||
                  layout == Strings.postCategoryLayoutSlideshow
...
class LayoutPostList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String? template =
        get(fields, ['template', 'template'], Strings.postItemDefault);
    Map<String, dynamic>? dataTemplate = get(fields, ['template', 'data'], {});
    double? width = ConvertData.stringToDouble(
        get(templateItem, ['data', 'size', 'width'], 100), 100);
    double? height = ConvertData.stringToDouble(
        get(templateItem, ['data', 'size', 'height'], 100), 100);

    // Styles config
    double? maxHeightImage =
        ConvertData.stringToDouble(get(styles, ['maxHeightImage'], 300), 300);
    ...
    Color dividerColor = ConvertData.fromRGBA(
        get(styles, ['dividerColor', themeModeKey], {}), Colors.transparent);
    ...
    switch (layout) {
      case Strings.postLayoutCarousel:
        return LayoutCarousel(
          posts: posts,
          buildItem: (_, {Post? post, int? index, double? widthView}) =>
              _buildItem(
            context,
            post: post,
            template: template,
