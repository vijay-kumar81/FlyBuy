...
class _IconScanState extends State<IconScan> {
  ...
  void onPressedScannerIcon(BuildContext context) async {
    ...
    if (option.isNotEmpty) {
      await _scanBarcodeNormal(
              scanMode: (option == "qrcode") ? ScanMode.QR : ScanMode.BARCODE)
          .then((value) async {
        if (value.isNotEmpty && value != '-1') {
          await _getIdProductWithBarcode(barcode: value).then((product) {
            if (product != null) {
              Navigator.pushNamed(
                  context, '${ProductScreen.routeName}/${product.id}',
  ...
  Future<String> _scanBarcodeNormal({ScanMode? scanMode}) async {
    ...
    try {
      barcodeScanRes = await FlutterBarcodeScanner.scanBarcode(
          '#ff6666', 'Cancel', true, scanMode ?? ScanMode.DEFAULT);
    } catch (_) {
      barcodeScanRes = '';
    }
  ...
  Future<Product?> _getIdProductWithBarcode({required String barcode}) async {
    Product? product;
    List<Product>? list =
        await _settingStore.requestHelper.getProducts(queryParameters: {
      "barcode": barcode,
