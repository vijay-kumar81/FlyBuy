...
class HeadingWidget extends StatelessWidget
  ...
  Widget build(BuildContext context) {
    ...
    //trailing
    String actionTitle =
        ConvertData.textFromConfigs(headingData['actionTitle'], languageKey);
    TextStyle actionTitleStyle = ConvertData.toTextStyle(
        get(headingData, ['actionTitle'], ''), themeModeKey);
    ...
    double? dividerHeight =
        ConvertData.stringToDouble(get(styles, ['dividerHeight'], 0.0));

    Color background = ConvertData.fromRGBA(
        get(styles, ['background', themeModeKey], {}), Colors.transparent);
    Color secondBackground = ConvertData.fromRGBA(
        get(styles, ['secondBackground', themeModeKey], {}),
        Colors.transparent);
    Color iconColor = ConvertData.fromRGBA(
        get(styles, ['iconColor', themeModeKey], {}), Colors.black);
    double? borderRadiusTopLeft =
        ConvertData.stringToDouble(get(styles, ['borderRadiusTopLeft'], 0));
    double? borderRadiusTopRight =
        ConvertData.stringToDouble(get(styles, ['borderRadiusTopRight'], 0));
    double? borderRadiusBottomLeft =
        ConvertData.stringToDouble(get(styles, ['borderRadiusBottomLeft'], 0));
    double? borderRadiusBottomRight =
        ConvertData.stringToDouble(get(styles, ['borderRadiusBottomRight'], 0));
    Color underlinedColor = ConvertData.fromRGBA(
    ...
    switch (layout) {
      case "divider":
        return Heading(
          ...
          dash: dividerHeight != 0
              ? Divider(
                  height: 2, thickness: dividerHeight, color: underlinedColor)
              : null,
          see: buildText(context, actionTitle, actionTitleStyle),
          trailing: enableIconAction == true
              ? FlybuyIconBuilder(data: iconAction, size: 16)
              : null,
        ...
        if (dividerHeight != null && dividerHeight > 0)
          Positioned(
            ...
            child: Container(
              ...
              decoration:
                  decorationColorImage(color: secondBackground).copyWith(
                borderRadius: BorderRadius.only(
                  topLeft: Radius.circular(borderRadiusTopLeft),
                  topRight: Radius.circular(borderRadiusTopRight),
                  bottomLeft: Radius.circular(borderRadiusBottomLeft),
