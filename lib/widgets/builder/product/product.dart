...
class ProductWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (_) {
      ...
      // Style
      double? height =
          ConvertData.stringToDoubleCanBeNull(get(styles, ['height'], null));
...
class _LayoutProductListState extends State<LayoutProductList>
    with GeneralMixin {
  @override
  Widget build(BuildContext context) {
  ...
  Widget buildLayout(BuildContext context, [double widthView = 300]) {
    ...
    double? width = ConvertData.stringToDouble(
        get(templateItem, ['data', 'size', 'width'], 100));
    double? height = ConvertData.stringToDouble(
        get(templateItem, ['data', 'size', 'height'], 100));
    int column = ConvertData.stringToInt(get(widget.styles, ['col'], 2), 2);
    double? ratio =
        ConvertData.stringToDouble(get(widget.styles, ['ratio'], 1), 1);

    // Styles config
    double? pad = ConvertData.stringToDouble(get(widget.styles, ['pad'], 0));
    double? dividerHeight =
        ConvertData.stringToDouble(get(widget.styles, ['dividerWidth'], 1));
    Color dividerColor = ConvertData.fromRGBA(
        get(widget.styles, ['dividerColor', widget.themeModeKey], {}),
        Colors.transparent);
  ...
  Widget _buildItem(
    ...
    // Template
    String? template = get(
        widget.fields, ['template', 'template'], Strings.productItemContained);
    Map<String, dynamic>? dataTemplate =
        get(widget.fields, ['template', 'data'], {});

    Color backgroundColor = ConvertData.fromRGBA(
        get(widget.styles, ['backgroundColorItem', widget.themeModeKey], {}),
        theme.cardColor);
    ...
    Color priceColor = ConvertData.fromRGBA(
        get(widget.styles, ['priceColor', widget.themeModeKey], {}),
        theme.textTheme.titleMedium?.color);
    Color salePriceColor = ConvertData.fromRGBA(
        get(widget.styles, ['salePriceColor', widget.themeModeKey], {}),
        ColorBlock.red);
    Color regularPriceColor = ConvertData.fromRGBA(
        get(widget.styles, ['regularPriceColor', widget.themeModeKey], {}),
        theme.textTheme.bodySmall?.color);
    Map wishlist =
        get(widget.styles, ['wishlistColor', widget.themeModeKey], {});
    ...
    Color labelNewColor = ConvertData.fromRGBA(
        get(widget.styles, ['labelNewColor', widget.themeModeKey], {}),
        ColorBlock.green);
    Color labelNewTextColor = ConvertData.fromRGBA(
        get(widget.styles, ['labelNewTextColor', widget.themeModeKey], {}),
        Colors.white);
    double? radiusLabelNew =
        ConvertData.stringToDouble(get(widget.styles, ['radiusLabelNew'], 10));
    Color labelSaleColor = ConvertData.fromRGBA(
        get(widget.styles, ['labelSaleColor', widget.themeModeKey], {}),
        ColorBlock.red);
    Color labelSaleTextColor = ConvertData.fromRGBA(
        get(widget.styles, ['labelSaleTextColor', widget.themeModeKey], {}),
        Colors.white);
    double? radiusLabelSale =
        ConvertData.stringToDouble(get(widget.styles, ['radiusLabelSale'], 10));

    double? radius =
        ConvertData.stringToDouble(get(widget.styles, ['radius'], 0));
    double? radiusImage =
        ConvertData.stringToDouble(get(widget.styles, ['radiusImage'], 8));

    String? typeCart = get(widget.styles, ['typeCart'], 'elevated');
    double? radiusCart =
        ConvertData.stringToDouble(get(widget.styles, ['radiusCart'], 8));
    Map? iconCart =
        get(widget.styles, ['iconCart'], {'name': 'plus', 'type': 'feather'});
    ...
    Color shadowColor = ConvertData.fromRGBA(
        get(widget.styles, ['shadowColor', widget.themeModeKey], {}),
        Colors.transparent);
    double offsetX =
        ConvertData.stringToDouble(get(widget.styles, ['offsetX'], 0));
    double offsetY =
        ConvertData.stringToDouble(get(widget.styles, ['offsetY'], 4));
    double blurRadius =
        ConvertData.stringToDouble(get(widget.styles, ['blurRadius'], 24));
    double spreadRadius =
        ConvertData.stringToDouble(get(widget.styles, ['spreadRadius'], 0));

    EdgeInsetsDirectional padding =
        ConvertData.space(paddingItem, 'paddingItem');
