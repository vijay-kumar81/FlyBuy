...
import 'package:flybuy/store/post_author/post_author_store.dart';
import 'package:flybuy/store/store.dart';
import 'package:flybuy/utils/utils.dart';
import 'package:flybuy/widgets/flybuy_post_author_item.dart';
...
class _PostAuthorWidgetState extends State<PostAuthorWidget>
  ...
  Widget build(BuildContext context) {
    ...
    double? height =
        ConvertData.stringToDouble(get(styles, ['height'], 300), 300);

    // Config general
    Map<String, dynamic> fields = widget.widgetConfig?.fields ?? {};
    int limit = ConvertData.stringToInt(get(fields, ['limit'], '4'));
    String? template =
        get(fields, ['template', 'template'], Strings.postAuthorItemVertical);
    ...
    return Observer(builder: (_) {
      ...
      List<PostAuthor> emptyPostAuthor =
          List.generate(limit, (index) => PostAuthor()).toList();
    ...
    switch (layout) {
      case Strings.postAuthorLayoutTwoColumn:
        return TwoColumnLayout(
          ...
          axisSpacing: pad,
          buildItem: (BuildContext context, int index, double width) =>
              buildItem(
            context,
  ...
  Widget buildItem(
    ...
    double? radiusItem =
        ConvertData.stringToDouble(get(styles, ['radiusItem'], 8));
