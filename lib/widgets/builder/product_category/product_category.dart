...
import 'package:flybuy/widgets/flybuy_product_category_item.dart';
...
class _ProductCategoryWidgetState extends State<ProductCategoryWidget>
    with CategoryMixin, Utility, ContainerMixin {
  ...
  Widget build(BuildContext context) {
    ...
    String layout =
        widget.widgetConfig?.layout ?? Strings.productCategoryLayoutList;
    ...
    return Observer(
      builder: (_) {
        ...
        int categoriesLength =
            categoriesList.length < limit ? categoriesList.length : limit;
        List<ProductCategory?> categories =
            categoriesList.sublist(0, categoriesLength);

        return Container(
          margin: ConvertData.space(margin, 'margin'),
          decoration: decorationColorImage(color: background),
          height: layout == Strings.productCategoryLayoutCarousel ||
                  layout == Strings.productCategoryLayoutSlideshow
              ? height
              : null,
          child: LayoutBuilder(
  ...
  Widget buildLayout(
    ...
    switch (layout) {
      case Strings.productCategoryLayoutCarousel:
        double? maxHeightImage = ConvertData.stringToDouble(
            get(styles, ['maxHeightImage'], 200), 200);
  ...
  Widget buildItem({
    ...
    Color subTextColor = ConvertData.fromRGBA(
        get(styles, ['subTextColor', themeModeKey], {}), Colors.black);
    double? radius = ConvertData.stringToDouble(get(styles, ['radius'], 0));
    double? radiusImage =
        ConvertData.stringToDouble(get(styles, ['radiusImage'], 0));
    double? fontSize =
        ConvertData.stringToDouble(get(styles, ['fontSize'], 16));

    return FlybuyProductCategoryItem(
      category: category,
      template: template,
