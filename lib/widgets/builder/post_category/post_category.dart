...
import 'package:flybuy/widgets/flybuy_post_category_item.dart';
...
class _PostCategoryWidgetState extends State<PostCategoryWidget>
  ...
  void didChangeDependencies() {
    ...
    if (widget.widgetConfig != null &&
      ...
      PostCategoryStore store = PostCategoryStore(_settingStore!.requestHelper,
          perPage: limit, lang: language, key: widget.widgetConfig!.id)
        ..getPostCategories();
  ...
  Widget build(BuildContext context) {
    ...
    // Layout
    String layout =
        widget.widgetConfig?.layout ?? Strings.postCategoryLayoutList;
    ...
    double heightImage =
        ConvertData.stringToDouble(get(styles, ['heightImage'], 200));
    ...
    // general config
    int limit = ConvertData.stringToInt(get(fields, ['limit'], 4));
    List<PostCategory> emptyCategories =
        List.generate(limit, (index) => PostCategory()).toList();
  ...
  Widget buildItem({
    ...
    Color background = ConvertData.fromRGBA(
        get(styles, ['backgroundItem', themeModeKey], {}), Colors.transparent);
    Color textColor = ConvertData.fromRGBA(
        get(styles, ['textColor', themeModeKey], {}), Colors.black);
    Color labelColor = ConvertData.fromRGBA(
        get(styles, ['labelColor', themeModeKey], {}), Colors.black);
    Color labelTextColor = ConvertData.fromRGBA(
        get(styles, ['labelTextColor', themeModeKey], {}), Colors.white);
    double? labelRadius =
        ConvertData.stringToDouble(get(styles, ['labelRadius'], 13));
    double? radius = ConvertData.stringToDouble(get(styles, ['radius'], 0));
    double? radiusImage =
        ConvertData.stringToDouble(get(styles, ['radiusImage'], 0));

    return FlybuyPostCategoryItem(
      category: category,
      width: width,
      height: height,
      heightImage: heightImage,
