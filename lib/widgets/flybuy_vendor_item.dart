...
class FlybuyVendorItem extends StatelessWidget with Utility, VendorMixin {
  final Vendor? vendor;
  ...
  Widget buildItem(BuildContext context,
      {double? maxWidth, double? widthScreen}) {
    ...
    switch (template) {
      case Strings.vendorItemEmerge:
        double width = ConvertData.stringToDouble(
            get(dataTemplate, ['sizeBanner', 'width'], 334), 334);
        double height = ConvertData.stringToDouble(
            get(dataTemplate, ['sizeBanner', 'height'], 174), 174);
        ...
        double heightSizeBanner = getHeightBanner(
            widthSizeBanner, widthBanner, heightBanner, width / height);
        return VendorEmergeItem(
          ...
          name: buildName(context,
              vendor: vendor, theme: theme, color: textColor),
          directionIcon: directionIcon,
          distance: enableDistance
              ? buildDistance(context, vendor: vendor, theme: theme)
              : null,
          rating: enableRating
              ? buildRating(context,
                  vendor: vendor, theme: theme, color: subTextColor)
              : null,
      ...
      case Strings.vendorItemContained:
        ...
        return VendorContainedItem(
          ...
          name: buildName(context,
              ...
              color: textColor),
          distance: enableDistance
              ? buildDistance(context,
                  vendor: vendor, theme: theme, isCenter: true)
              : null,
          rating: enableRating
              ? buildRating(context,
                  vendor: vendor,
      ...
      case Strings.vendorItemGradient:
        double width = ConvertData.stringToDouble(
            get(dataTemplate, ['sizeBanner', 'width'], 334), 334);
        double height = ConvertData.stringToDouble(
            get(dataTemplate, ['sizeBanner', 'height'], 180), 180);
        ...
        return VendorGradientItem(
          ...
          banner: buildImage(context,
              ...
              fit: BoxFit.cover),
          name: buildName(context,
              vendor: vendor, theme: theme, color: textColor ?? Colors.white),
          distance: enableDistance
              ? buildDistance(context,
                  vendor: vendor, theme: theme, color: Colors.white)
              : null,
          rating: enableRating
              ? buildRating(context,
                  vendor: vendor, theme: theme, color: subTextColor)
          ...
          padding: padding as EdgeInsets? ??
              const EdgeInsets.symmetric(
                  horizontal: 22, vertical: itemPaddingMedium),
          borderRadius: borderRadius,
          border: border,
          boxShadow: boxShadow ?? initBoxShadow,
          color: color ?? theme.colorScheme.surface,
  ...
  double getHeightBanner(
      double? widthBannerSize, double? width, double? height, double ratio) {
