...
import 'package:flybuy/widgets/flybuy_star.dart';

class FlybuyRating extends StatelessWidget {
  final double value;
  final int count;
  final double size;
  final Color? color;
  ...
  const factory FlybuyRating.select({
    Key? key,
    int defaultRating,
    int count,
    double size,
  ...
  const factory FlybuyRating.number({
    Key? key,
    double value,
    double iconSize,
    double fontSize,
  ...
  Widget build(BuildContext context) {
    return FlybuyStar(star: getStar);
  }
}

class _FlybuyRatingSelect extends FlybuyRating {
  final int defaultRating;
  final Function(int value)? onFinishRating;
  final Color? selectColor;

  const _FlybuyRatingSelect({
    Key? key,
    this.onFinishRating,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: List.generate(count, (index) {
        IconData icon = index < defaultRating
...
class _FlybuyRatingNumber extends FlybuyRating {
  final Color? iconColor;
  final Color? textColor;
  final double fontSize;
  final double iconSize;

  const _FlybuyRatingNumber({
    Key? key,
    double value = 2.5,
    this.iconSize = 12,
    this.fontSize = 14,
      ...
      children: [
        ...
        Padding(
          ...
          child: Icon(FontAwesomeIcons.solidStar,
              size: iconSize, color: iconColor ?? ColorBlock.yellow),
        ),
      ],
    );
  }
