...
class StarSolid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Padding(
      ...
      padding: EdgeInsets.only(right: 4),
      child:
          Icon(FontAwesomeIcons.solidStar, size: 12, color: ColorBlock.yellow),
    );
  }
}
...
class FlybuyStar extends StatelessWidget {
  final String star;

  const FlybuyStar({Key? key, required this.star}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children:
          starList.containsKey(star) ? starList[star]! : [const SizedBox()],
    );
  }
}
