          ...
          return StreamBuilder<PositionData>(
            ...
            builder: (context, snapshot) {
              ...
              return Row(
                children: [
                  SizedBox(
                      width: position.inMilliseconds.toDouble() > 3599070.0
                          ? 39
                          : 33,
                      child: Text(
                          formatPosition(position: position) ?? '$position',
                          style: widget.stylePosition)),
                  const Text('/'),
                  Padding(
                    padding: const EdgeInsets.only(left: 4.0, right: 6),
                    child: Text(
                        formatPosition(position: duration) ?? '$duration',
                        style: widget.styleDuration),
                  ),
                ],
              );
            },
