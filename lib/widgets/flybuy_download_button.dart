...
import 'package:flybuy/models/download/download.dart';
...
class HandleDownloadController extends DownloadController with ChangeNotifier {
  ...
  String _getFileName(Download download) {
    ...
    String extensionFile =
        arrFile.isNotEmpty ? arrFile[arrFile.length - 1] : 'jpg';
  ...
  String _checkExistFile(String fileName, String fileExtension) {
    ...
    if (File('$_localPath$newFileName').existsSync()) {
      ...
      if (regex.hasMatch(fileName)) {
        ...
        number = int.parse(
            regex.firstMatch(fileName)?.group(1) ?? number.toString());
      }
      do {
        number++;
  ...
  Future<void> _handleDownload(Download download, String token) async {
    ...
    notifyListeners();
    _onCallbackDownload(
        name: locationFileDownload.replaceFirst(_localPath, ''));
  }
}

class FlybuyDownloadButton extends StatelessWidget {
  final DownloadStatus status;
  final VoidCallback onDownload;
  final VoidCallback onCancel;
  final VoidCallback onOpen;

  const FlybuyDownloadButton({
    Key? key,
...
@immutable
class FlybuyDownloadButtonLoading extends StatelessWidget {
  const FlybuyDownloadButtonLoading({
    Key? key,
