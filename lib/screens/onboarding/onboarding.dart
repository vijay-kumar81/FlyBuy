...
class _OnboardingScreenState extends State<OnBoardingScreen>
  ...
  Widget buildScreen({
    ...
    // Configs
    Data? data =
        dataScreen.screens != null && dataScreen.screens!['onBoarding'] != null
            ? dataScreen.screens!['onBoarding']
            : null;
    WidgetConfig? widgetConfig =
        data != null ? data.widgets!['onBoardingPage'] : null;
    ...
    Color backgroundColor = ConvertData.fromRGBA(
        get(styles, ['backgroundItem', themeModeKey], {}),
        theme.scaffoldBackgroundColor);
    Color skipColor = ConvertData.fromRGBA(
        get(styles, ['skipColor', themeModeKey], {}),
        theme.textTheme.bodySmall?.color);
    Color indicatorColor = ConvertData.fromRGBA(
        get(styles, ['indicatorColor', themeModeKey], {}), theme.dividerColor);
          ...
          ? SizedBox(
              ...
              child: Swiper(
                ...
                itemBuilder: (BuildContext context, int index) {
                  dynamic item = items.elementAt(index) is Map
                      ? get(items.elementAt(index), ['data'], {})
                      : null;
                  ...
                  builder: (_, SwiperPluginConfig? config) {
                    ...
                    return Align(
                      ...
                      child: Container(
                        ...
                        padding:
                            paddingHorizontal.copyWith(top: itemPaddingMedium),
  ...
  Widget buildItem(
    ...
    Color titleColor = ConvertData.fromRGBA(
        get(styles, ['titleColor', themeModeKey], {}),
        theme.textTheme.headlineSmall?.color);
    Color subtitleColor = ConvertData.fromRGBA(
        get(styles, ['subtitleColor', themeModeKey], {}),
        theme.textTheme.bodyMedium?.color);
    double? titleSize =
        ConvertData.stringToDouble(get(styles, ['titleSize'], 28));
    double? subtitleSize =
        ConvertData.stringToDouble(get(styles, ['subtitleSize'], 14));

    switch (layout) {
      case 'overlay':
        double opacity =
            ConvertData.stringToDouble(get(styles, ['opacity'], 0.9));
        Color gradientFrom = ConvertData.fromRGBA(
            get(styles, ['gradientFrom', themeModeKey], {}),
            Colors.transparent);
        Color gradientTo = ConvertData.fromRGBA(
            get(styles, ['gradientTo', themeModeKey], {}), ColorBlock.gray3);
        Color dividerColor = ConvertData.fromRGBA(
            get(styles, ['dividerColor', themeModeKey], {}),
        ...
        return Stack(
          children: [
            FlybuyCacheImage(image, width: width, height: height),
            Positioned(
              top: 0,
              bottom: 0,
              left: 0,
      ...
      default:
        ...
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            FlybuyCacheImage(image, width: widthImage, height: heightImage),
            Expanded(
              child: ListView(
