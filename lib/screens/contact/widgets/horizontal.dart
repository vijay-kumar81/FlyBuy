...
class HorizontalContact extends StatelessWidget with AppBarMixin, Utility {
  ...
  final Function(
          double lat, double lng, double bearing, double tilt, double zoom)
      gotoMarker;
  ...
  void goAddress(dynamic item) {
    double? lat = ConvertData.stringToDouble(
        get(item, ['data', 'lat'], initLat), initLat);
    double? lng = ConvertData.stringToDouble(
        get(item, ['data', 'lng'], initLng), initLng);
    double? bearing = ConvertData.stringToDouble(
        get(item, ['data', 'bearing'], enableBearing), enableBearing);
    double? tilt = ConvertData.stringToDouble(
        get(item, ['data', 'tilt'], enableTilt), enableTilt);
    double? zoom = ConvertData.stringToDouble(
        get(item, ['data', 'zoom'], enableZoom), enableZoom);
                  ...
                  child: Row(
                    children: List.generate(
                      ...
                      (index) {
                        ...
                        // info
                        String heading = get(
                            item, ['data', 'titleHeading', languageKey], '');

                        EdgeInsetsDirectional padding =
                            EdgeInsetsDirectional.only(
                                end: index < count - 1 ? 16 : 0);
                        double? widthItem = count < 2 ? width - 40 : null;

                        return Padding(
                          padding: padding,
                          child: ContactContainedItem(
                            headOffice: Text(heading,
...
class Info extends StatelessWidget with Utility {
  ...
  List<ItemInfo> getListItem() {
    ...
    String? phoneData =
        get(item, ['data', 'titlePhone', languageKey], '0123456789');
    ...
    return [
      ...
      if (emailData != '' && emailData != null)
        ItemInfo(
            icon: iconMailData,
            text: emailData,
            action: () => launch('mailto:$emailData')),
      ...
      children: List.generate(data.length, (index) {
        ...
        return Padding(
          ...
          child: InkWell(
            ...
            child: Row(
              children: [
                FlybuyIconBuilder(data: value.icon, size: 16),
