...
import 'package:flybuy/screens/auth/forgot_screen.dart';
...
class _LoginScreenState extends State<LoginScreen>
    with AppBarMixin, LoadingMixin, SnackMixin, Utility, GeneralMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (_) {
      ...
      WidgetConfig widgetConfig =
          widget.store!.data!.screens!['login']!.widgets!['login']!;

      // Get configs
      Map<String, dynamic>? configsLogin =
          widget.store!.data!.screens!['login']!.configs;
      Color appbarColor = ConvertData.fromRGBA(
          get(configsLogin, ['appbarColor', widget.store!.themeModeKey]),
          Colors.white);
      bool extendBodyBehindAppBar =
          get(configsLogin, ['extendBodyBehindAppBar'], false);
      ...
      Color background = ConvertData.fromRGBA(
          get(stylesLogin, ['background', widget.store!.themeModeKey]),
          Colors.white);

      // Layout
      String layout = widgetConfig.layout ?? Strings.loginLayoutSocialTop;

      bool enableRegister = widget.store != null
          ? getConfig(widget.store!, ['enableRegister'], true)
          : true;
      ...
      return Scaffold(
        ...
        body: Stack(
          children: [
            buildLayout(
                layout, widgetConfig, background, enableRegister, translate),
  ...
  Widget buildLayout(
    ...
    bool? loginPhoneNumber =
        get(widgetConfig.fields, ['loginPhoneNumber'], true);
    ...
    switch (layout) {
      case Strings.loginLayoutLogoTop:
        return LoginScreenLogoTop(
          ...
          padding: padding,
          paddingFooter: const EdgeInsetsDirectional.only(
      ...
      default:
        return LoginScreenSocialTop(
          ...
          header: HeadingText.animated(
              title: translate('login_txt_login'), enable: enable),
          loginForm: LoginForm(handleLogin: _handleLogin),
          ...
          termText:
              TermText(mainAxisAlignment: MainAxisAlignment.start, html: term),
          registerText: register,
        );
    }
  }
...
class _TextRegister extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        Text(translate('login_description_register'),
