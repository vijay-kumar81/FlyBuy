...
class FilterList extends StatelessWidget {
  ...
  List<Map<String, dynamic>> getFilter(BuildContext context) {
    ...
    ProductCategory? valueFilterCategory =
        filterStore.categories.isNotEmpty ? filterStore.categories[0] : null;
    if (valueFilterCategory != null &&
        (category == null || valueFilterCategory.id != category!.id)) {
      filters.add({
        ...
        'onDeleted': () {
          filterStore.clearCategory(
              categories: category != null ? [category!] : null);
          filter!
              .clearCategory(categories: category != null ? [category!] : null);
        },
      });
    }

    if (filterStore.brand != null &&
        (brand == null || filterStore.brand?.id != brand?.id)) {
      filters.add({
        ...
        'text': translate(
          'product_list_min_price',
          {
            'price': formatCurrency(context,
                price: '${filterStore.rangePrices.end}'),
          },
        ),
        'onDeleted': () {
    ...
    if (filterStore.rangePrices.end < filter!.productPrices.maxPrice! &&
        filterStore.rangePrices.end > 0) {
      filters.add({
        'text': translate(
                  ...
                  ...List.generate(filters.length, (index) {
                    return Padding(
                      padding: EdgeInsetsDirectional.only(
                          end: index < filters.length - 1 ? 8 : 0),
                      child: SizedBox(
                        height: 34,
                        child: InputChip(
                          label: Row(
