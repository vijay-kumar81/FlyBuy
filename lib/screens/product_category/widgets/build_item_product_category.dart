...
class BuildItemProductCategory extends StatefulWidget {
  ...
  @override
  State<BuildItemProductCategory> createState() =>
      _BuildItemProductCategoryState();
}

class _BuildItemProductCategoryState extends State<BuildItemProductCategory>
    with CategoryMixin, TransitionMixin {
  ...
  Widget buildItem(
    ...
    Color background = ConvertData.fromRGBA(
        get(widget.styles, ['backgroundItem', widget.themeModeKey], {}),
        theme.cardColor);
    double? sizeText =
        ConvertData.stringToDouble(get(widget.styles, ['sizeText'], 16));
    Color textColor = ConvertData.fromRGBA(
        get(widget.styles, ['textColor', widget.themeModeKey], {}),
        Colors.white);
    double? sizeSubtext =
        ConvertData.stringToDouble(get(widget.styles, ['sizeSubtext'], 12));
    Color subtextColor = ConvertData.fromRGBA(
        get(widget.styles, ['subtextColor', widget.themeModeKey], {}),
        Colors.white);
    double? radiusItem =
        ConvertData.stringToDouble(get(widget.styles, ['radiusItem'], 8));
    double? radiusImage =
        ConvertData.stringToDouble(get(widget.styles, ['radiusImage'], 0));

    String? typeTemplate = get(
        widget.template, ['template'], Strings.productCategoryItemHorizontal);
    ...
    bool isCheckSubcategoryScreen =
        enableNavigate ? false : isSubcategory && enableSubcategories;
    ...
    switch (typeTemplate) {
      case Strings.productCategoryItemCard:
        ...
        Color borderColor = ConvertData.fromRGBA(
            get(dataTemplate, ['borderColor', widget.themeModeKey], {}),
            Colors.white);

        // size image
        double size = widget.heightItem is double && widget.heightItem! < 60
            ? widget.heightItem!
            : 60;
        ...
        return ProductCategoryCardItem(
          ...
          name: buildNameProduct(
            textName: textName,
            nameStyle: theme.textTheme.titleMedium
                ?.copyWith(fontSize: sizeText, color: textColor),
          ),
          count: enableNumber
              ? buildCount(
                  category: widget.category,
                  style: theme.textTheme.bodySmall
                      ?.copyWith(fontSize: sizeSubtext, color: subtextColor),
                  translate: translate,
                )
              : null,
          padding: EdgeInsets.symmetric(
              horizontal: itemPaddingMedium, vertical: padVertical),
          color: background,
      ...
      case Strings.productCategoryItemOverlay:
        // setting template
        bool enableNumber = get(dataTemplate, ['enableNumber'], true);
        TextAlign alignment = ConvertData.toTextAlignDirection(
            get(dataTemplate, ['alignment'], 'center'));
        Color opacityColor = ConvertData.fromRGBA(
            get(dataTemplate, ['opacityColor', widget.themeModeKey], {}),
            Colors.black);
        double opacity =
      ...
      case Strings.productCategoryItemGrid:
        ...
        bool enableNumber = get(dataTemplate, ['enableNumber'], true);
        // item sub category
        int countSubcategory = ConvertData.stringToInt(
            get(dataTemplate, ['maxCountSubcategory'], 6));
        int colSubcategory = ConvertData.stringToInt(
            get(dataTemplate, ['columnSubcategory'], 3));
        ...
        return ProductCategoryGridItem(
          name: buildNameProduct(
            nameStyle: theme.textTheme.titleMedium
                ?.copyWith(fontSize: sizeText, color: textColor),
            countStyle: theme.textTheme.bodySmall
                    ?.copyWith(fontSize: sizeSubtext, color: subtextColor) ??
      ...
      case Strings.productCategoryItemBasic:
        ...
        Color borderColor = ConvertData.fromRGBA(
            ...
            theme.dividerColor);
        double? iconSize =
            ConvertData.stringToDouble(get(dataTemplate, ['iconSize'], 14));
        Color iconColor = ConvertData.fromRGBA(
            get(dataTemplate, ['iconColor', widget.themeModeKey], {}),
      ...
      case Strings.productCategoryItemHorizontalOver:
        ...
        // size image
        double widthImage =
            widget.widthItem is double && widget.widthItem! / 2 < 120
                ? widget.widthItem! / 2
                : 120;
        double heightImage =
            widget.heightItem is double && widget.heightItem! < 92
                ? widget.heightItem!
                : 92;

        bool enableEndImage = alignmentView == 'right' ||
            (alignmentView == 'zigzag' && widget.index % 2 == 1);
      ...
      default:
        ...
        // size image
        double widthImage =
            widget.widthItem is double && widget.widthItem! / 2 < 92
                ? widget.widthItem! / 2
                : 92;
        double? heightImage =
            widget.heightItem is double && widget.heightItem! < 92
  ...
  Widget buildNameProduct(
    ...
    return Text(textName ?? widget.category!.name!,
        style: nameStyle, textAlign: textAlign);
  }
}
