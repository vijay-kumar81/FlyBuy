...
class RehubComparisonTable extends StatelessWidget with Utility {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (_, BoxConstraints constraints) {
        ...
        double widthItem =
            innerBlocks!.length < 2 ? widthView : widthView * 0.8;
        ...
        return SingleChildScrollView(
          ...
          child: Table(
            ...
            children: <TableRow>[
              if (enableBadges!)
                buildBadge(innerBlocks: innerBlocks, theme: theme),
              ...
              if (enableSpec!)
                buildInfo(context,
                    ...
                    nameValue: 'specText'),
              if (enableCallout!)
                buildButtonCallout(context,
                    innerBlocks: innerBlocks, theme: theme),
            ],
          ),
        );
      },
    ...
    return TableRow(
      children: <Widget>[
        ...
        ...List.generate(innerBlocks.length, (index) {
          Map dataBlock = innerBlocks.elementAt(index) is Map
              ? innerBlocks.elementAt(index)
              : {};
          Map? attrs = get(dataBlock, ['attrs'], {}) is Map
              ? get(dataBlock, ['attrs'], {})
              : {};
          ...
          Color? colorBadge = ConvertData.fromHex(
              get(attrs, ['badgeColor'], ''), Colors.transparent);
          if (enableBadge) {
            return Container(
              color: colorBadge,
    ...
    return TableRow(
      children: <Widget>[
        ...
        ...List.generate(innerBlocks.length, (index) {
          ...
          // number
          int number =
              ConvertData.stringToInt(get(attrs, ['numberValue'], 1), 1);
          Color? numberColor = ConvertData.fromHex(
              get(attrs, ['numFlybuyr'], ''), theme!.primaryColor);
          ...
          // rating
          double? starRating =
              ConvertData.stringToDouble(get(attrs, ['starRating'], 5), 5);

          // listFlybuy
          String? listTitle = get(attrs, ['listTitle'],
              translate('post_detail_comparison_list_item'));
          List? listItems = get(attrs, ['listItems'], []);

          // button
          String? buttonText = get(attrs, ['buttonText'],
              translate('post_detail_comparison_button'));
          Color? buttonColor = ConvertData.fromHex(
              get(attrs, ['buttonColor'], ''), theme.primaryColor);
                    ...
                    children: [
                      if (enableImage! && linkImage!.isNotEmpty)
                        Padding(
                          ...
                          child: FlybuyCacheImage(linkImage,
                              width: widthImage,
                              height: heightImage,
                              fit: BoxFit.contain),
                        ),
                      ...
                      if (enableSubtitle! && productSubtitle!.isNotEmpty)
                        Padding(
                          padding:
                              const EdgeInsets.only(bottom: itemPaddingSmall),
                      ...
                      if (enableStars!)
                        Padding(
                          padding:
                              const EdgeInsets.only(bottom: itemPaddingSmall),
                          child: FlybuyRating(value: starRating),
                        ),
                      if (enableList!)
                        Column(
                          children: [
  ...
  TableRow buildButtonCallout(BuildContext context,
      {required List innerBlocks, ThemeData? theme}) {
    TranslateType translate = AppLocalizations.of(context)!.translate;
    return TableRow(
      children: <Widget>[
        Container(),
