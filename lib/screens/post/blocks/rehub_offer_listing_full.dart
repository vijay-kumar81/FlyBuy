...
class OfferListingFull extends StatelessWidget with Utility, BlockMixin {
  ...
  Widget build(BuildContext context) {
    ...
    Color? numberbgcolor = ConvertData.fromHex(
        get(attrs, ['numberbgcolor'], ''), ColorBlock.green);
    Color? numbercolor =
        ConvertData.fromHex(get(attrs, ['numbercolor'], ''), Colors.white);
    Color? highlightcolor = ConvertData.fromHex(
        get(attrs, ['highlightcolor'], ''), ColorBlock.blue);
    Color? disclaimerbg = ConvertData.fromHex(
        get(attrs, ['disclaimerbg'], ''), theme.colorScheme.surface);
    Color? disclaimercolor = ConvertData.fromHex(
        get(attrs, ['disclaimercolor'], ''),
        theme.textTheme.titleMedium?.color);
    Color? titleColor = ConvertData.fromHex(
        get(attrs, ['titleColor'], ''), theme.textTheme.titleMedium?.color);
    Color? priceColor = ConvertData.fromHex(
        get(attrs, ['priceColor'], ''), ColorBlock.redBase2);
    Color? expandColor = ConvertData.fromHex(
        get(attrs, ['expandcolor'], ''), theme.textTheme.titleMedium?.color);
    Color? expandBg =
        ConvertData.fromHex(get(attrs, ['expandbg'], ''), theme.cardColor);
      ...
      builder: (_, BoxConstraints constraints) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (_, int index) {
            ...
            Color? colorBadge = ConvertData.fromHex(
                get(attrs, ['customBadge', 'textColor'], ''), Colors.white);
            Color? backgroundBadge = ConvertData.fromHex(
                get(attrs, ['customBadge', 'backgroundColor'], ''),
                ColorBlock.blue);
            ...
            bool checkExpire = expirationDate.isNotEmpty
                ? compareSpaceDate(date: expirationDate, space: 0)
                : true;
            ...
            Widget itemWidget = ListingBuilderItem(
              ...
              score: score.isNotEmpty
                  ...
                  : null,
              content: copy.isNotEmpty
                  ? Text(copy, style: theme.textTheme.bodyMedium)
                  : null,
              price: buildPrice(
              ...
              disclaimer: disclaimer.isNotEmpty
                  ? Text(disclaimer,
                      style: theme.textTheme.bodySmall
                          ?.copyWith(color: disclaimercolor))
                  : null,
              expand: enableExpand! &&
                      expandlabel!.isNotEmpty &&
                      expandcontent!.isNotEmpty
                  ? buildExpand(
                      context,
                      label: expandlabel,
