...
class RehubHowTo extends StatelessWidget with Utility {
    ...
    return StepItem(
      visit: Container(
        ...
        decoration: BoxDecoration(
          border:
              Border.all(width: 2, color: colorBorder ?? theme.dividerColor),
  ...
  Widget build(BuildContext context) {
    ...
    Color? titleColor = get(attrs, ['titleColor'], '') != ''
        ? ConvertData.fromHex(get(attrs, ['titleColor'], ''))
        : null;
    Color? borderColor = get(attrs, ['borderColor'], '') != ''
        ? ConvertData.fromHex(get(attrs, ['borderColor'], ''))
        : null;
    Color? numberColor = get(attrs, ['numberColor'], '') != ''
        ? ConvertData.fromHex(get(attrs, ['numberColor'], ''))
        : null;
    Color? numberBgColor = get(attrs, ['numberBgColor'], '') != ''
        ? ConvertData.fromHex(get(attrs, ['numberBgColor'], ''))
        : null;
