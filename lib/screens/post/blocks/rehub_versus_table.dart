...
class RehubVersusTable extends StatelessWidget with Utility {
  ...
  Widget buildItem(
    ...
    if (type == 'image') {
      ...
      child = FlybuyCacheImage(image, width: size, height: size);
    }
    if (type == 'tick') {
      child = const Icon(FeatherIcons.checkCircle,
          size: 22, color: ColorBlock.green);
    }
    if (type == 'times') {
  ...
  Widget build(BuildContext context) {
    ...
    String heading =
        get(attrs, ['heading'], translate('post_detail_versus_title'));
    String subheading =
        get(attrs, ['subheading'], translate('post_detail_versus_subline'));
    ...
    Color? bg = get(attrs, ['bg'], '') != ''
        ? ConvertData.fromHex(get(attrs, ['bg'], ''))
        : null;
    Color? color = get(attrs, ['color'], '') != ''
        ? ConvertData.fromHex(get(attrs, ['color'], ''))
        : null;
      ...
      subtitle: subheading.isNotEmpty
          ? Text(
              ...
              style: theme.textTheme.bodyMedium
                  ?.copyWith(color: color?.withOpacity(0.5)),
              textAlign: TextAlign.center,
            )
          : null,
      column: column,
