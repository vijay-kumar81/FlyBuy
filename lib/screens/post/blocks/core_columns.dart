...
class Columns extends StatelessWidget with Utility {
  ...
  Widget build(BuildContext context) {
    ...
    String strBackground =
        attrs is Map ? get(attrs, ['style', 'color', 'background'], '') : '';
    // String strText = attrs is Map ? get(attrs, ['style', 'color', 'text'], '') : 'null';
    Color? background = strBackground.isNotEmpty
        ? ConvertData.fromHex(strBackground, Colors.transparent)
        : null;
      ...
      child: LayoutBuilder(
        builder: (_, BoxConstraints constraints) {
          ...
          for (int i = 0; i < innerBlocks.length; i++) {
            widgetBlocks.add(buildItem(context,
                item: innerBlocks.elementAt(i), widthView: widthView));
  ...
  Widget buildItem(BuildContext context, {dynamic item, double? widthView}) {
    ...
    double? percent = percentWidth != null && percentWidth is String
        ? ConvertData.stringToDouble(
            percentWidth.substring(0, percentWidth.length - 1))
        : null;
