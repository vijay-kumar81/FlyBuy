...
class RehubReviewbox extends StatelessWidget with Utility {
  ...
  Widget buildCriterias(BuildContext context,
      {required List data, Color? selectColor}) {
          ...
          return TableRow(
            children: [
              ...
              TableCell(
                ...
                child: buildItemBottom(
                  pad: padItem,
                  child: FlybuyAnimationIndicator(
                    value: value / 10,
  ...
  Widget buildScore(
    ...
    return Column(
      children: [
        Stack(
          children: [
            FlybuyAnimationIndicator(
              value: value / 10,
              indicatorColor: color,
              size: 72,
              type: FlybuyAnimationIndicatorType.circle,
            ),
            Container(
              width: 72,
              height: 72,
  ...
  Widget build(BuildContext context) {
    ...
    Color? mainColor =
        ConvertData.fromHex(get(attrs, ['mainColor'], ''), ColorBlock.redBase3);
    String title =
        get(attrs, ['title'], translate('post_detail_reviewbox_title'));
    String description = get(
        attrs, ['description'], translate('post_detail_reviewbox_description'));
    double scoreManual =
        ConvertData.stringToDouble(get(attrs, ['scoreManual'], 0), 0);
    ...
    String? prosTitle =
        get(attrs, ['prosTitle'], translate('post_detail_positive'));
    String? consTitle =
        get(attrs, ['consTitle'], translate('post_detail_negatives'));
    ...
    return ReviewBox(
      ...
      description: description.isNotEmpty
          ...
          : null,
      criteria: criterias.isNotEmpty
          ? buildCriterias(context, data: criterias, selectColor: mainColor)
          : null,
