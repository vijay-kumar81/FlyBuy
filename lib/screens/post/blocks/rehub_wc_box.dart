...
class _RehubWcBoxState extends State<RehubWcBox>
    with Utility, SingleTickerProviderStateMixin, WishListMixin {
  ...
  Widget buildViewInfo(BuildContext context,
      {Map? attrs, String? type, double? width}) {
    ...
    switch (type) {
      case 'product':
        ...
        double? regularPrice =
            ConvertData.stringToDouble(get(attrs, ['regularPrice'], 0), 0);
        double? salePrice =
            ConvertData.stringToDouble(get(attrs, ['salePrice'], 0), 0);
        ...
        children.add(
          WoocommerceBoxProductItem(
            ...
            price: priceWidget,
            description: Text(
                description.replaceAll(RegExp(r"\s+\b|\b\s"), " "),
            ...
            band: Row(
              children: [
                Expanded(
                    child: Text(brandList, style: theme.textTheme.bodySmall)),
                const InkWell(
      ...
      case 'specifiction':
        ...
        if (productAttributes is String && productAttributes.isNotEmpty) {
          double? fontSize = theme.textTheme.bodyMedium?.fontSize;
          ...
          children.add(ClipRRect(
            ...
            child: Html(
              ...
              style: {
                'html': styleTag.copyWith(
                    backgroundColor: theme.colorScheme.surface),
                'body': styleTag,
                'p': styleTag.copyWith(
                    margin:
                        const EdgeInsets.only(left: secondItemPaddingSmall)),
                'div': styleTag,
                // 'img': styleTag,
                'table': Style(
  ...
  Widget build(BuildContext context) {
    ...
    Map? attrs = get(widget.block, ['attrs'], {}) is Map
        ? get(widget.block, ['attrs'], {})
        : {};
    ...
    return Theme(
      ...
      child: LayoutBuilder(
        builder: (_, BoxConstraints constraints) {
          ...
          double itemWidth =
              maxWidth != double.infinity ? maxWidth : screenWidth;
            ...
            children: [
              ...
              SizedBox(
                ...
                child: TabBarView(
                  ...
                  children: List.generate(
                    data.length,
                    (index) => buildViewInfo(context,
                        attrs: attrs, type: data[index], width: itemWidth),
                  ),
                ),
              ),
            ],
