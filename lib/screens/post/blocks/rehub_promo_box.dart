...
class RehubPromoBox extends StatelessWidget with Utility {
  ...
  Widget build(BuildContext context) {
    ...
    String content =
        get(attrs, ['content'], translate('post_detail_sample_content'));
    String? buttonText =
        get(attrs, ['buttonText'], translate('post_detail_purchase_now'));
    String? buttonLink = get(attrs, ['buttonLink'], '');

    Color? backgroundColor =
        ConvertData.fromHex(get(attrs, ['backgroundColor'], '#f8f8f8'));
    Color? textColor =
        ConvertData.fromHex(get(attrs, ['textColor'], '#333333'));
    ...
    Color? borderColor =
        ConvertData.fromHex(get(attrs, ['borderColor'], '#dddddd'));
    Color? highlightColor =
        ConvertData.fromHex(get(attrs, ['highlightColor'], '#fb7203'));
    String? highlightPosition = get(attrs, ['highlightPosition'], 'Left');
    double borderSize =
        ConvertData.stringToDouble(get(attrs, ['borderSize'], 1));
      ...
      button: showButton
          ? SizedBox(
              ...
              child: ElevatedButton(
                ...
                style:
                    ElevatedButton.styleFrom(padding: paddingHorizontalLarge),
                child: Text(buttonText!.isNotEmpty
                    ? buttonText
                    : translate('post_detail_buy_item')),
              ),
            )
          : null,
