...
import 'package:flybuy/models/builder/product_detail_value.dart';
import 'package:flybuy/models/post/post.dart';
import 'package:flybuy/screens/product/widgets/product_custom.dart';
...
class PostBlock extends StatelessWidget with PostMixin, ContainerMixin {
  ...
  List<Widget> buildColumn(
    ...
    return columns.map((e) {
      ProductDetailValue configs =
          ProductDetailValue.fromJson(get(e, ['value'], {}));
      ...
      Color foreground = ConvertData.fromRGBA(
          get(e, ['value', 'foreground', themeModeKey]), Colors.transparent);
  ...
  Widget buildBlock(String type, String queryBy, ProductDetailValue configs,
      String textHtml) {
    switch (type) {
        ...
        return Column(
          children: [
            LayoutBuilder(
              ...
              double widthImage =
                  maxWidth != double.infinity ? maxWidth : width;
      ...
      case PostBlocks.content:
        if (!enableBlock) {
          return FlybuyHtml(html: post?.content?.rendered ?? '');
        }
