...
import 'package:flybuy/extension/list.dart';
...
import 'package:flybuy/screens/wishlist/widget/button_delete.dart';
...
class _WishListScreenState extends State<WishListScreen>
    with NavigationMixin, LoadingMixin, GeneralMixin, AppBarMixin {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      auto = autorun(
        (_) {
          List<String> wishlistIds = _wishListStore!.data
              .map((element) => element.toString())
              .toList();
          if (!showedWishlist.equals(wishlistIds) &&
              wishlistIds.length >= showedWishlist.length) {
      ...
      react = reaction((p0) => _productsStore.products, (p0) {
        List<String> products = _productsStore.products
  ...
  void didChangeDependencies() {
    ...
    List<Product> productWishList = _wishListStore!.data
        .map((String id) => Product(id: int.parse(id)))
        .toList();
  ...
  Future<void> _reloadWishlist(List<String> wishlistIds) async {
    List<Product> productWishList =
        wishlistIds.map((String id) => Product(id: int.parse(id))).toList();
    String? key = StringGenerate.getProductKeyStore(
      'wishlist_product',
      includeProduct: productWishList,
      currency: _settingStore.currency,
  ...
  void dismissItem(
    ...
    final snackBar = SnackBar(
      content: Text(AppLocalizations.of(context)!
          .translate('wishlist_notification_deleted', {'name': product.name})),
                      ...
                      children: [
                        Expanded(
                          child: ButtonSlidable(
                            ...
                            onPressed: () =>
                                dismissItem(context, index, product),
                          ),
                        )
                      ],
                    ),
