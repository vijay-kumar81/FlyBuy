...
class _QuantityFieldState extends State<QuantityField> with Utility {
  ...
  void initState() {
    ...
    if ((!checkEmptyValue && qty == null) ||
        (qty != null && qty <= widget.min)) {
      enableMinus = false;
    }
    if (widget.max != 0 &&
        ((!checkEmptyValue && qty == null) ||
            (qty != null && qty >= widget.max))) {
      enablePlus = false;
    }
  ...
  _onChange(BuildContext context, String str) {
    ...
    double duration =
        adjustDuration ? getDuration(context, option: widget.item) : 0;
