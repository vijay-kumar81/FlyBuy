...
class _ViewDropdownState extends State<_ViewDropdown> {
  ...
  void didChangeDependencies() {
    ...
    String text = widget.required
        ? translate('product_select_option')
        : translate('product_none');
    if (value != null) {
      dynamic option = widget.options.firstWhereOrNull(
          (element) => widget.options.indexOf(element) == value.visit);
  ...
  void didUpdateWidget(covariant _ViewDropdown oldWidget) {
    if (widget.value != oldWidget.value) {
      ...
      String text = widget.required
          ? translate('product_select_option')
          : translate('product_none');
      String textOld = oldWidget.required
          ? translate('product_select_option')
          : translate('product_none');
      ...
      if (oldValue != null) {
        dynamic oldOption = oldWidget.options.firstWhereOrNull(
            (element) => oldWidget.options.indexOf(element) == oldValue.visit);
            ...
            child: Column(
              children: [
                ...
                ...List.generate(widget.options.length, (index) {
                  ...
                  return FlybuyTile(
                    title: Text(getTextField(option),
                        style: index == value?.visit
                            ? activeTitleStyle
                            : titleStyle),
                    trailing: index == value?.visit
                        ? Icon(FeatherIcons.check,
                            size: 20, color: theme.primaryColor)
                        : null,
                    isChevron: false,
      ...
      children: [
        ...List.generate(options.length, (index) {
          ...
          return FlybuyTile(
            ...
            title: Text(label),
            trailing: (showPrice && priceText.isNotEmpty) ||
                    (showDuration && durationText.isNotEmpty)
...
class _ViewImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dynamic optionSelect =
        data?.visit != null && data!.visit >= 0 && data.visit < options.length
            ? options.elementAt(data.visit)
            : null;
    String price =
        optionSelect is Map ? getTextPrice(context, option: optionSelect) : '';
    String duration = optionSelect is Map
        ? getTextDuration(context, option: optionSelect, translate: translate)
        : '';
          ...
          SingleChildScrollView(
            ...
            child: Row(
              ...
              children: List.generate(options.length, (index) {
                ...
                double padEnd =
                    index < options.length - 1 ? itemPaddingSmall : 0;
...
class _ImageItemState extends State<_ImageItem> with LoadingMixin {
  ...
  Widget buildImage() {
    ...
    return FlybuyCacheImage(_url, width: 40, height: 40);
  }
