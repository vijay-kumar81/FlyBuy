      ...
      if (required && !checkExistValue(data[keyValue])) {
        ...
        if (type == 'custom_text') {
          String restrictionsType =
              get(item, ['restrictions_type'], 'any_text');
...
bool checkExistValue(dynamic value) {
  if (value is AddOnData) {
    return (value.type == AddOnDataType.string &&
            value.string?.value != null) ||
        (value.type == AddOnDataType.option && value.option?.value != null) ||
        (value.type == AddOnDataType.listOption &&
            value.listOption?.isNotEmpty == true);
  }
  return false;
}
...
String? getErrorText(
    String value, String restrictionsType, TranslateType translate) {
  if (value.isEmpty) return null;
  if (restrictionsType == 'only_letters') {
    return letterValidator(
        value: value, errorLetter: translate('product_addon_letter'));
  }
  if (restrictionsType == 'only_numbers') {
    return numberValidator(
        value: value, errorNumber: translate('product_addon_number'));
  }
  if (restrictionsType == 'only_letters_numbers') {
    return letterNumberValidator(
        value: value,
        errorLetterNumber: translate('product_addon_letter_number'));
  }
  if (restrictionsType == 'email') {
    return emailValidator(
        value: value, errorEmail: translate('product_addon_email'));
  }
  return null;
}

String? getErrorNumber(
    String value, int min, int max, TranslateType translate) {
  if (value.isEmpty) return null;
