...
import 'package:flybuy/screens/product/widgets/product_image_popup.dart';
import 'package:flybuy/store/setting/setting_store.dart';
import 'package:flybuy/utils/utils.dart';
import 'package:flybuy/widgets/flybuy_sirv_image.dart';
...
class _ProductSlideshowState extends State<ProductSlideshow>
    with ProductSlideshowPagination, Utility, LoadingMixin {
  ...
  Future<void> _getVideoLinkFromMetaData() async {
    if (widget.product != null &&
      ...
      // Rehub theme
      Map<String, dynamic> videosRehubTheme =
          widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == 'rh_product_video',
        orElse: () => {'value': null},
      );
      if (videosRehubTheme['value'] != null &&
          videosRehubTheme['value'] != '') {
        List<String> videos =
            LineSplitter.split(videosRehubTheme['value']).toList();
        int i = 0;
      ...
      Map<String, dynamic> productVideoForWoo =
          widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == 'afpv_featured_video_type',
        orElse: () => {'value': ''},
      );

      Map<String, dynamic> productVideoForWooFeatured =
          widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == 'afpv_enable_featured_video_product_page',
        orElse: () => {'value': ''},
      );

      Map<String, dynamic> productVideoForThumb =
          widget.product!.metaData!.firstWhere(
      ...
      // Vimeo
      Map<String, dynamic> productVideoForWooVimeo =
          widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == 'afpv_vm_featured_video_id',
        orElse: () => {'value': ''},
      );

      if (productVideoForWooType == 'vimeo' &&
          productVideoForWooVimeo['value'] != '') {
      ...
      // Youtube
      Map<String, dynamic> productVideoForWooYoutube =
          widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == 'afpv_yt_featured_video_id',
        orElse: () => {'value': ''},
      );

      if (productVideoForWooType == 'youtube' &&
          productVideoForWooYoutube['value'] != '') {
        await _parserVideoUrl(
      ...
      // Custom video
      Map<String, dynamic> productVideoForCustom =
          widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == 'afpv_cus_featured_video_id',
        orElse: () => {'value': ''},
      );

      if (productVideoForWooType == 'custom' &&
    ...
    // Sirv image
    Map<String, dynamic> sirvImage = widget.product!.metaData!.firstWhere(
        (e) => get(e, ['key'], '') == '_sirv_woo_gallery_data',
        orElse: () => {'value': ''});
    ...
    if (sirvImage['value'] != '') {
      ...
      List<Map<String, dynamic>> images =
          List<Map<String, dynamic>>.of(_images);
  ...
  /// Get Id video
  Future<void> _parserVideoUrl(String url, String urlThumb,
      {bool featured = false}) async {
    List<Map<String, dynamic>> images = List<Map<String, dynamic>>.of(_images);

    // Parser Youtube video
    if (VideoParserUrl.isValidFullYoutubeUrl(url) ||
        VideoParserUrl.isValidSortYoutubeUrl(url)) {
  ...
  Future<Map<String, dynamic>?> _getVimeoThumb(String videoId) async {
    try {
      final dio = Dio();
      Response response =
          await dio.get('https://player.vimeo.com/video/$videoId/config');
      Map<String, dynamic> data = response.data;

      String linkBasic =
          get(data, ['video', 'thumbs', 'base'], Assets.noImageUrl);
      String linkSmall =
          get(data, ['video', 'thumbs', '960'], Assets.noImageUrl);
      String linkTiny =
          get(data, ['video', 'thumbs', '640'], Assets.noImageUrl);
  ...
  Widget build(BuildContext context) {
    ...
    double containerHeight = widget.height == 0
        ? screenHeight
        : (screenWidth * widget.height!) / widget.width!;
    ...
    Map<String, dynamic>? indicatorMargin =
        get(styles, ['indicatorMargin'], null);
    ...
    return ConstrainedBox(
      constraints:
          BoxConstraints.expand(height: containerHeight, width: screenWidth),
      child: Swiper(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          String urlImage =
              get(image, [widget.productGalleryThumbSizes], urlImageDefault);
          String type = get(image, ['type'], '');

          /// Check is video
          bool isVideo =
              type == 'youtube' || type == 'vimeo' || type == 'custom';

          return Builder(
            builder: (BuildContext context) {
              /// Image Sirv
              if (type == 'sirv') {
                return FlybuySirvImage(video: video);
              }
        ...
        pagination: FlybuyPaginationSwiper(
          alignment: indicatorAlignment,
          margin: ConvertData.space(
              indicatorMargin, 'indicatorMargin', EdgeInsetsDirectional.zero),
  ...
  SwiperPlugin _buildPagination(BuildContext context) {
    ...
    String? productGalleryIndicator =
        get(styles, ['productGalleryIndicator'], 'dot');
    ...
    double? indicatorActiveSize =
        ConvertData.stringToDouble(get(styles, ['indicatorActiveSize'], 10));
    ...
    double? indicatorBorderRadius =
        ConvertData.stringToDouble(get(styles, ['indicatorBorderRadius'], 8));
    ...
    if (productGalleryIndicator!.toLowerCase() == 'number') {
      return numberPagination(
        textStyle:
            theme.textTheme.labelSmall?.apply(color: colorIndicatorActive),
