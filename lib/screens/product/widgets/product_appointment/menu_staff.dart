...
import 'package:flybuy/models/product/staff_model.dart';
import 'package:flybuy/screens/product/add_on/label.dart';
...
class _ViewDropdownState extends State<_ViewDropdown> {
  ...
  void didChangeDependencies() {
    ...
    if (widget.value != null) {
      dynamic option = widget.options
          .firstWhereOrNull((element) => element.id == widget.value?.id);
      text = getName(option);
    }
  ...
  String getName(StaffModel? option) {
    ...
    if (option?.price != null && option!.price! > 0) {
      text +=
          '${option.name} +${formatCurrency(context, price: option.price.toString())}';
    } else {
      text += option?.name ?? '';
    }
    return text;
  ...
  void didUpdateWidget(covariant _ViewDropdown oldWidget) {
    if (widget.value != oldWidget.value) {
      ...
      if (oldWidget.value != null) {
        dynamic oldOption = oldWidget.options
            .firstWhereOrNull((element) => element.id == oldWidget.value?.id);
      ...
      builder: (BuildContext context) {
        ...
        return Container(
          ...
          child: buildViewModal(
            title: widget.titleSelect ??
                AppLocalizations.of(context)!.translate('product_list_select'),
            child: Column(
              children: List.generate(widget.options.length, (index) {
                StaffModel option = widget.options[index];
                return FlybuyTile(
                  title: Text(getName(option),
                      style: option.id == widget.value?.id
                          ? activeTitleStyle
                          : titleStyle),
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      decoration:
          const InputDecoration(suffixIcon: Icon(Icons.keyboard_arrow_down)),
