...
Map getACFields(Product? product) {
  ...
  if (meta?.isNotEmpty == true) {
    Map<String, dynamic>? barcode = meta!.firstWhereOrNull(
        (element) => get(element, ['key'], '') == '_ywbc_barcode_image');

    Map<String, dynamic>? labelBarcode = meta.firstWhereOrNull((element) =>
        get(element, ['key'], '') == 'ywbc_barcode_display_value_custom_field');
...
class ProductAdvancedFieldsCustom extends StatelessWidget with Utility {
  ...
  const ProductAdvancedFieldsCustom(
      {Key? key, this.product, this.align, this.fieldName})
      : super(key: key);
