...
class _ProductBottomBarState extends State<ProductBottomBar>
  ...
  Widget build(BuildContext context) {
    bool enableBottomBarSearch =
        get(widget.configs, ['enableBottomBarSearch'], false);
    bool enableBottomBarHome =
        get(widget.configs, ['enableBottomBarHome'], false);
    bool enableBottomBarShare =
        get(widget.configs, ['enableBottomBarShare'], true);
    bool enableBottomBarWishList =
        get(widget.configs, ['enableBottomBarWishList'], true);
    bool enableBottomBarCart =
        get(widget.configs, ['enableBottomBarCart'], true);
    bool enableBottomBarAddToCart =
        get(widget.configs, ['enableBottomBarAddToCart'], true);
    bool enableBottomBarQty =
        get(widget.configs, ['enableBottomBarQty'], false);
    bool enableBuyNow = get(widget.configs, ['enableBuyNow'], false);
    bool enableChat = get(widget.configs, ['enableChat'], false);
    bool expressCheckoutButton =
        get(widget.configs, ['expressCheckoutButton'], false);
    ...
    bool showQty =
        enableBottomBarQty && widget.product!.type != productTypeExternal;
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (!enableBottomBarAddToCart ||
                (!enableBottomBarAddToCart && !showBottomNow))
              const Spacer(),
            ...
            if (enableBottomBarAddToCart)
              addToCartBottom(context, showQty, showBottomNow, isOutOfStock),
            if (showBottomNow)
              buyNowBottom(
                  context, showQty, enableBottomBarAddToCart, isOutOfStock),
            if (enableChat) chatVendor(context),
            if (showQty && enableBottomBarAddToCart && showBottomNow)
              const SizedBox(width: 20),
            if (expressCheckoutButton)
              SizedBox(
                  height: 48,
                  child: FlybuyExpressCheckout(
  ...
  Widget addToCartBottom(BuildContext context, bool enableQty,
      bool enableBuyNow, bool disableStock) {
          ...
          child: buttonAdd == 1 && widget.loading!
              ...
              : Text(
                  widget.product!.type == productTypeExternal
                      ? widget.product!.buttonText != null &&
