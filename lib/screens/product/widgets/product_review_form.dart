...
import 'package:flybuy/widgets/flybuy_captcha.dart';
...
class _ProductReviewFormState extends State<ProductReviewForm>
          ...
          onPressed: () async {
            try {
              final pickedFileList =
                  await _picker.pickMultiImage(imageQuality: 50, maxWidth: 600);
  ...
  Widget build(BuildContext context) {
    ...
    Map<String, dynamic>? data =
        _settingStore?.data?.settings!['general']!.widgets!['general']!.fields;

    bool enableCaptchaReviewProduct =
        get(data, ['enableCaptchaReviewProduct'], true);
                  ...
                  Form(
                    ...
                    child: Column(
                      children: [
                        ...
                        SizedBox(
                            height: _imageFileList.isNotEmpty
                                ? itemPadding
                                : itemPaddingLarge),
                        buildPreviewImages(),
                        buildPhotos(),
    ...
    return Row(
      children: [
        buildSelectButton(
          ...
          icon:
              Icon(FeatherIcons.camera, color: Theme.of(context).primaryColor),
          title: 'Add Image',
        ),
        if (!enableAddVideo) ...[
          const SizedBox(
          ...
          buildSelectButton(
            icon:
                Icon(FeatherIcons.video, color: Theme.of(context).primaryColor),
            title: 'Add Video',
          ),
        ]
      ],
  ...
  Widget buildSelectButton(
      {GestureTapCallback? onTap, Widget? icon, String? title}) {
    return Expanded(
      child: InkWell(
        onTap: onTap,
        child: DottedBorder(
