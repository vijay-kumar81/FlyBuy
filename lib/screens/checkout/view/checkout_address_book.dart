...
class _CheckoutAddressBookState extends State<CheckoutAddressBook>
    with Utility {
  ...
  void didChangeDependencies() {
    ...
    List<AddressBookData>? data =
        widget.type == 'shipping' ? widget.data.shipping : widget.data.billing;
    AddressBookData? initAddress = data
        ?.firstWhereOrNull((element) => element.name == widget.valueSelected);

    String text = initAddress != null
        ? getConvertAddress(initAddress.address ?? '')
        : translate('checkout_new_address');
    setState(() {
        ...
        child: Column(
          ...
          children: data.isNotEmpty == true
              ? List.generate(data.length, (index) {
                  ...
                  return FlybuyTile(
                    title: Text(getConvertAddress(item.address ?? ''),
