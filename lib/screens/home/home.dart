...
import 'package:flybuy/screens/no_internet/no_internet_mixin.dart';
import 'package:flybuy/service/messaging.dart';
...
class _HomeScreenState extends State<HomeScreen>
    with Utility, MessagingMixin, NavigationMixin, NoInternetMixin {
    ...
    return Observer(
      builder: (_) {
        final WidgetConfig widgetConfig =
            widget.store!.data!.settings!['general']!.widgets!['general']!;
        ...
        bool enableSelectLanguage =
            get(widgetConfig.fields, ['enableSelectLanguage'], false);
        bool enableLocationScreen =
            get(widgetConfig.fields, ['enableLocationScreen'], false);
        bool enableOnBoarding =
            get(widgetConfig.fields, ['enableOnBoarding'], true);
        bool forceLogin = get(widgetConfig.fields, ['forceLogin'], false);
        bool forceLoginMobile =
            get(widgetConfig.fields, ['forceLoginMobile'], false);
  ...
  Widget buildHome(DataScreen? data) {
    ...
    extraScreens.forEach((key, value) {
      newWidgetOptions.putIfAbsent(
          "extraScreens_$key", () => CustomScreen(screenKey: key, type: "tab"));
    });

    // Drawer
    String? layout = sidebarData.widgets!['sidebar']!.layout;
    Color background = ConvertData.fromRGBA(
      get(sidebarData.widgets!['sidebar']!.styles,
          ['background', Provider.of<SettingStore>(context).themeModeKey]),
    ...
    if (scaleTop == '$layout') {
      heightStyle =
          -MediaQuery.of(context).size.height * (isRTL ? -0.19 : 0.19);
    }
