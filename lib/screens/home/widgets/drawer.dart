...
class Sidebar extends StatelessWidget with CategoryMixin, Utility {
  ...
  List<ProductCategory?>? showHierarchy(
      {List<ProductCategory>? categories, hierarchy = true}) {
  ...
  Widget build(BuildContext context) {
    ...
    String? imageBg =
        get(widgetConfig.styles, ['imageBg', 'src'], Assets.noImageUrl);
    ...
    Color color = ConvertData.fromRGBA(
      get(widgetConfig.styles,
          ['color', Provider.of<SettingStore>(context).themeModeKey]),
    ...
    Color borderColor = ConvertData.fromRGBA(
      get(widgetConfig.styles,
          ['borderColor', Provider.of<SettingStore>(context).themeModeKey]),
    ...
    return Theme(
      data: Theme.of(context).copyWith(
        ...
        scrollbarTheme:
            ScrollbarThemeData(thickness: MaterialStateProperty.all<double>(0)),
      ),
      child: Stack(
        children: [
          Container(
