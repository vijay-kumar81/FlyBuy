...
class _RefineState extends State<Refine> {
  ...
  void onChangeCategory({PostCategory? postCategory, selected = false}) {
    List<PostCategory?> categorySelected =
        List<PostCategory?>.of(_categorySelected);
          ...
          children: [
            Padding(
              padding: const EdgeInsets.only(
                  top: itemPaddingLarge, bottom: itemPaddingMedium),
              child: Stack(
                children: [
                  Center(
                      child: Text(translate('refine'),
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () => Navigator.pop(context,
                      {'categories': _categorySelected, 'tags': _tagSelected}),
                ),
              ),
            )
          ],
...
class __CategoriesState extends State<_Categories> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        FlybuyTile(
          title: FlybuyText(translate('categories')),
        ...
        if (_expand)
          ...List.generate(widget.categories!.length, (index) {
            ...
            bool checkSelect = widget.categorySelected!
                    .indexWhere((e) => e!.id == category.id) >=
                0;
            Color? textColor =
                checkSelect ? theme.textTheme.titleMedium?.color : null;
            return FlybuyTile(
              leading: FlybuyRadio.iconCheck(isSelect: checkSelect),
              title: RichText(
                text: TextSpan(
                  text: category.name,
                  children: [
              ...
              onTap: () => widget.onChange!(
                  postCategory: category, selected: !checkSelect),
            );
          }),
      ],
    );
...
class __TagsState extends State<_Tags> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        FlybuyTile(
          title: FlybuyText(translate('post_list_tags')),
          trailing: _IconButton(
            active: _expand,
            onPressed: () => setState(() {
              _expand = !_expand;
        ...
        if (_expand)
          ...List.generate(widget.tags!.length, (index) {
            ...
            bool isSelect =
                widget.tagSelected!.indexWhere((e) => e!.id == tag.id) >= 0;
            Color? textColor =
