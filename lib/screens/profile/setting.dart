...
class _SettingScreenState extends State<SettingScreen> with AppBarMixin {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (BuildContext context) {
        ...
        return buildViewModal(
            title: translate('app_settings_language'),
            child: ModalLanguage(settingStore: _settingStore));
      },
  ...
  String? getCodeLanguage() {
    int indexSelect = _settingStore!.supportedLanguages
        .indexWhere((element) => element.locale == _settingStore!.locale);
    Language languageSelect =
        _settingStore!.supportedLanguages[indexSelect > 0 ? indexSelect : 0];
    ...
    return Scaffold(
      ...
      body: Observer(
        builder: (_) => ListView(
          ...
          children: [
            FlybuyTile(
              title: Text(translate('app_settings_language'),
                  style: theme.textTheme.titleSmall),
              trailing:
                  Text(getCodeLanguage()!, style: theme.textTheme.bodyLarge),
              onTap: () => onClickLanguage(),
            ),
            if (_settingStore!.currencies.length > 1)
              FlybuyTile(
                title: Text(translate('app_settings_currency'),
                    style: theme.textTheme.titleSmall),
                trailing: Text(_settingStore!.currency!,
                    style: theme.textTheme.bodyLarge),
                onTap: () => onClickCurrency(),
              ),
            if (enableChangeTheme)
              FlybuyTile(
                title: Text(translate('app_settings_dark_theme'),
                    style: theme.textTheme.titleSmall),
                trailing: CupertinoSwitch(
                  value: _settingStore!.darkMode,
                  onChanged: (value) =>
                      _settingStore!.setDarkMode(value: value),
                ),
                isChevron: false,
              ),
            if (_authStore?.isLogin == true && enableUsingBiometric)
              FlybuyTile(
                title: Text(translate('login_local_using_biometric'),
                    style: theme.textTheme.titleSmall),
                trailing: CupertinoSwitch(
                  value: _settingStore!.enableUsingBiometric,
                  onChanged: (value) =>
                      _settingStore!.setBiometric(value: value),
                ),
                isChevron: false,
              ),
            const ClearCache(),
