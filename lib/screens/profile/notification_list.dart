...
import 'package:flybuy/widgets/flybuy_notification_item.dart';
...
class _NotificationListState extends State<NotificationList>
    with AppBarMixin, LoadingMixin, NavigationMixin, Utility {
  ...
  void _onScroll() {
    if (!_controller.hasClients ||
        _messageStore.loading ||
        !_messageStore.canLoadMore) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: baseStyleAppBar(
        context,
        title:
            AppLocalizations.of(context)!.translate('notifications_list_title'),
        actions: [
          Padding(
      ...
      body: Observer(
        builder: (_) {
          ...
          List<MessageData> loadingNotification = List.generate(
                  10, (index) => MessageData(id: '', createdAt: '', seen: 0))
              .toList();

          List<MessageData> messData =
              isShimmer ? loadingNotification : messages;
          return Stack(
            children: [
                    ...
                    if (_messageStore.loading && _messageStore.canLoadMore)
                      SliverToBoxAdapter(
                        child: buildLoading(context,
                            isLoading: _messageStore.canLoadMore),
                      ),
                  ],
                ),
                if (_messageStore.messages.isEmpty && !_messageStore.loading)
                  _buildNotificationEmpty(),
              ],
            ],
          );
  ...
  Widget buildItem(int index, MessageData message, bool isShimmer) {
    return Dismissible(
      ...
      key: UniqueKey(),
      direction:
          isShimmer ? DismissDirection.none : DismissDirection.horizontal,
      ...
      onDismissed: (direction) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(AppLocalizations.of(context)!
              .translate('notifications_list_delete')),
      ...
      child: Column(
        children: [
          FlybuyNotificationItem(message: message),
          const Padding(
    ...
    String? result = await showDialog<String>(
      ...
      builder: (BuildContext context) => AlertDialog(
        title: Text(AppLocalizations.of(context)!
            .translate('notifications_list_dialog_title')),
        content: Text(AppLocalizations.of(context)!
            .translate('notifications_list_dialog_description')),
        actions: <Widget>[
          TextButton(
            onPressed: () => Navigator.pop(context, 'Cancel'),
            child: Text(AppLocalizations.of(context)!
                .translate('notifications_list_dialog_cancel')),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context, 'OK'),
            child: Text(AppLocalizations.of(context)!
                .translate('notifications_list_dialog_ok')),
          ),
        ],
      ),
    );
    ...
    return NotificationScreen(
      ...
      content: Text(
          AppLocalizations.of(context)!
              .translate('notifications_you_currently'),
          style: Theme.of(context).textTheme.bodyMedium),
      iconData: FeatherIcons.bell,
      textButton:
          Text(AppLocalizations.of(context)!.translate('notifications_back')),
