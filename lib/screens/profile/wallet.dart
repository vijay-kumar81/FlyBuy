...
class _WalletScreenState extends State<WalletScreen>
  ...
  void _onScroll() {
    if (!_controller.hasClients ||
        _transactionStore.loading ||
        !_transactionStore.canLoadMore) return;
    ...
    return Observer(
      builder: (_) {
        List<TransactionWallet> transactionWallets =
            _transactionStore.transactionWallets;
        bool loading = _transactionStore.loading;

        bool isShimmer = transactionWallets.isEmpty && loading;
        List<TransactionWallet> loadingProduct =
            List.generate(10, (index) => TransactionWallet()).toList();

        List<TransactionWallet> data =
            isShimmer ? loadingProduct : transactionWallets;
        return Scaffold(
                ...
                sliver: SliverToBoxAdapter(
                  child: Column(
                    children: [
                      ...
                      Text(translate('wallet_balance_subtitle'),
                      ...
                      Column(
                        children: List.generate(data.length, (index) {
                          ...
                          return Padding(
                            ...
                            child:
                                FlybuyTransactionWalletItem(item: data[index]),
                          );
                        }),
                      ),
                    ],
              ...
              if (loading)
                SliverToBoxAdapter(
                  child: buildLoading(context,
                      isLoading: _transactionStore.canLoadMore),
                ),
            ],
          ),
        );
