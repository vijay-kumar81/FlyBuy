...
class _DeleteAccountScreenState extends State<DeleteAccountScreen>
    with TickerProviderStateMixin, SnackMixin {
  ...
  void deleteAccount(String otp) async {
    try {
      ...
      Map<String, String>? item = reasonData.firstWhereOrNull(
          (element) => get(element, ['value'], '') == _reason);

      Map<String, dynamic> data =
          await _settingStore.requestHelper.deleteAccount(
        data: {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: TabBarView(
        ...
        children: [
          ReasonStep(
              reason: _reason, changeReason: changeReason, nextStep: nextStep),
          TermStep(
            nextStep: () {
              changeAgree(false);
              nextStep();
