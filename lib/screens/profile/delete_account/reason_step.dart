...
class _ReasonStepState extends State<ReasonStep> with Utility, AppBarMixin {
  ...
  void didChangeDependencies() {
    if (widget.reason?.isNotEmpty == true) {
      ...
      Map<String, String>? item = reasonData.firstWhereOrNull(
          (element) => get(element, ['value'], '') == widget.reason);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FlybuyFixedBottom(
        childBottom: FlybuyFixedBottomContainer(
                  ...
                  child: GestureDetector(
                    onTap: () async {
                      String? data = await showModalBottomSheet<String>(
                        ...
                        builder: (BuildContext context) {
                          return buildViewModal(
                            title: translate(
                                'delete_account_reason_dropdown_label'),
                            child: Column(
                              children:
                                  List.generate(reasonData.length, (index) {
                                Map<String, String> item = reasonData[index];
                                TextStyle? titleStyle =
                                    theme.textTheme.titleSmall;
                                TextStyle? activeTitleStyle = titleStyle
                      ...
                      if (data != null && data != _value) {
                        Map<String, String>? item = reasonData.firstWhereOrNull(
                            (element) => get(element, ['value'], '') == data);
                        setState(() {
                          _value = data;
                          _controller.text =
                              translate(get(item, ['label_name'], ''));
                        });
                    ...
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        enabled: false,
                        labelText:
                            translate('delete_account_reason_dropdown_label'),
                        hintText: translate(
                            'delete_account_reason_dropdown_placeholder'),
