...
import 'package:flybuy/filters/filter_price.dart' show b2bkingFilterPrice;
import 'package:flybuy/filters/filter_quantity.dart';
import 'package:flybuy/models/product/product.dart';
import 'package:flybuy/models/product/product_category.dart';
...
mixin ProductMixin {
  ...
  double getPercent(double salePrice, double regularPrice) {
    if (salePrice > 0 && salePrice <= regularPrice) {
      return ((regularPrice - salePrice) * 100) /
          (regularPrice == 0 ? 1 : regularPrice);
    }
    return 0;
  }
  ...
  Widget buildPrice(
    ...
    TextStyle? stylePrice =
        priceStyle is TextStyle ? priceStyle : theme.textTheme.titleSmall;
    TextStyle? styleSale = saleStyle is TextStyle
        ? saleStyle
        : theme.textTheme.titleSmall?.copyWith(color: ColorBlock.red);
    TextStyle? styleRegular =
        regularStyle is TextStyle ? regularStyle : theme.textTheme.titleSmall;
    TextStyle? stylePercent = stylePercentSale is TextStyle
        ? stylePercentSale
    ...
    if (product.type == productTypeVariable ||
        product.type == productTypeGrouped) {
        ...
        return Html(
          ...
          style: {
            ...
            'del': Style.fromTextStyle(
                styleRegular!.copyWith(decoration: TextDecoration.lineThrough)),
    ...
    String? filterRegularPrice =
        b2bkingFilterPrice(context, price, product, 'regular');
    String? filterSalePrice =
        b2bkingFilterPrice(context, salePrice, product, 'sale');

    if ((product.onSale != null &&
            product.onSale! &&
            filterSalePrice == salePrice) ||
        (filterSalePrice != null &&
            filterSalePrice != '' &&
            filterSalePrice != '0')) {
      TranslateType translate = AppLocalizations.of(context)!.translate;
      double percent = getPercent(ConvertData.stringToDouble(salePrice),
          ConvertData.stringToDouble(price));
      String value = percent
          .toStringAsFixed(percent.truncateToDouble() == percent ? 0 : 1);
  ...
  Widget buildTagExtra(
    ...
    if (enableNew == true &&
        (product.date?.isNotEmpty == true &&
            compareSpaceDate(date: product.date!, space: spaceTimeNew))) {
      children.add(
        BadgeUi(
          text: Text(
  ...
  Widget buildRating(
    ...
    return Wrap(
      ...
      children: [
        if (showNumber)
          FlybuyRating.number(value: rating, textColor: color)
        else
          Padding(
            padding: const EdgeInsets.only(bottom: 3),
            child: FlybuyRating(value: rating),
          ),
  ...
  Widget? buildQuantityItem(
    ...
    int? max = b2bkingFilterQuantity(
        context,
        stockQty != null && stockQty > 0 ? stockQty : FlybuyQuantity.maxQty,
        product,
        "max");
  ...
  Widget? buildQueryData(
    ...
    if (checkField) {
      ...
      if (type == "image") {
        double widthImage =
            ConvertData.stringToDouble(get(query, ["width"], 100));
        double heightImage =
            ConvertData.stringToDouble(get(query, ["height"], 100));
        ...
        String? url =
            getStringQuery(context, product?.rawData, query, languageKey);
        if (url != null) {
