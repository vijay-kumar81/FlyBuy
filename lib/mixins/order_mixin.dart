...
mixin OrderMixin {
  ...
  Widget buildDate(ThemeData theme, OrderData order, String locale) {
    ...
    return Text(formatDate(date: order.dateCreated!, locate: locale),
        style: theme.textTheme.bodySmall);
  }
  ...
  Widget buildStatus(
      ThemeData theme, TranslateType translate, OrderData order) {
    if (order.id == null) {
      return FlybuyShimmer(
        child: Container(
          height: 18,
    ...
    String textStatus = status.isNotEmpty
  ...
  Widget buildCancel(
      ThemeData theme, TranslateType translate, OrderData order, Widget child) {
    if (order.id == null) {
      return FlybuyShimmer(
        child: Container(
          height: 18,
