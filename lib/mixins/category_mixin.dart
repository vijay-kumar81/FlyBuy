...
abstract class CategoryMixin {
  /// Exclude categories
  List<ProductCategory?>? exclude(
      {List<ProductCategory?>? categories, required List<int> excludes}) {
    ...
    for (ProductCategory? productCategory in categories) {
      if (!excludes.contains(productCategory!.id)) {
        newCategories.add(ProductCategory(
          ...
          categories: exclude(
              categories: productCategory.categories, excludes: excludes),
        ));
      }
    }
  ...
  /// Include categories
  List<ProductCategory?>? include(
      {List<ProductCategory?>? categories, required List<int> includes}) {
    ...
    for (ProductCategory? productCategory in categories) {
      List<ProductCategory?>? subs =
          include(categories: productCategory!.categories, includes: includes);
  ...
  List<ProductCategory?> parent(
      {required List<ProductCategory?> categories, int? parentId = 0}) {
    if (categories.isEmpty) return categories;
    ...
    for (ProductCategory? productCategory in categories) {
      ...
      newCategories.addAll(
          parent(categories: productCategory.categories!, parentId: parentId));
    }
    return newCategories;
  }
  ...
  Widget buildCount({
    ...
    String text = category?.count is int && category!.count! > 1
        ...
        : translate!(
            'product_category_item', {'count': '${category?.count ?? 0}'});
    return Text(text,
        style: style ?? theme!.textTheme.bodySmall, textAlign: textAlign);
  }

  Widget buildNamePostCategory(
    ...
    return Text(category.name!,
        style:
            theme!.textTheme.titleSmall?.merge(style).copyWith(color: color));
  }
