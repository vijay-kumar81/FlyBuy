...
abstract class CheckoutStoreBase with Store {
  ...
  Future<void> updateBillingFromMap({
    ...
    String country = place.addressComponents
            .firstWhereOrNull(
    ...
    billing['city'] = place.addressComponents
            .firstWhereOrNull((element) =>
                element.types?.contains('administrative_area_level_2') ?? false)
            ?.shortName ??
        '';
  ...
  Future<dynamic> progressServer(
      {String? cartKey, required Map<String, dynamic> data}) async {
    try {
      loadingPayment = true;
  ...
  Future<void> changeAddress(
    ...
    if ((shipToDifferentAddress &&
            shipping != null &&
            keys.any(
                (String key) => shipping[key] != oldShippingAddress[key])) ||
  ...
  Future<void> updateAddress() async {
    await _cartStore.updateCustomerCart(data: {
