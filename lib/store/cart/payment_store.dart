...
abstract class PaymentStoreBase with Store {
  ...
  Future<void> getGateways() async {
    try {
      ...
      List<Gateway> gatewaysEnable =
          data.map((g) => Gateway.fromJson(g)).toList().cast<Gateway>();
      gateways = ObservableList<Gateway>.of(gatewaysEnable
          .where((element) => element.enabled && supported.contains(element.id))
          .toList());
      loading = false;
    } on DioException {
      loading = false;
