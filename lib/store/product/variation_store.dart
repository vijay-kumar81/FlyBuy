...
abstract class VariationStoreBase with Store {
  ...
  @observable
  ObservableMap<String?, String?> _selected =
  ...
  bool get canAddToCart =>
      _data != null &&
      _selected.keys.isNotEmpty &&
      _selected.keys.length == _data!['attribute_ids'].keys.length;

  @computed
  Map<String, dynamic> get findVariation => canAddToCart
      ? _data!['variations']?.firstWhere((e) {
          Map<String, String> attributes =
              Map<String, String>.from(e['attributes']);
          return _selected.keys.every(
            (el) {
              String key = el!.toLowerCase();
              return attributes[key] == null ||
                  attributes[key] == '' ||
                  attributes[key] == _selected[el];
            },
          );
        }, orElse: () => {'id': 0})
      : {'id': 0};
  ...
  Future<void> getVariation(
      List<Map<String, dynamic>>? defaultAttributes) async {
    _loading = true;
    try {
      ...
      ProductVariable res = await _requestHelper!
          .getProductVariations(queryParameters: preQueryParameters(qs));
      ...
      if (defaultAttributes?.isNotEmpty == true) {
        for (var att in defaultAttributes!) {
          ...
          String? keyAttr =
              res.ids?.keys.firstWhere((k) => res.ids?[k] == idAttr);
          if (keyAttr != null) {
            selectData.addAll({
              keyAttr: valueAttr,
            });
        ...
        Map<String, dynamic>? variationSelected =
            data['variations']?.firstWhere((e) {
          Map<String, String> attributes =
              Map<String, String>.from(e['attributes']);
        ...
        if (id == null) {
          ...
          if (variation is Map && variation['attributes'] is Map) {
            selectData = Map.castFrom<dynamic, dynamic, String?, String?>(
