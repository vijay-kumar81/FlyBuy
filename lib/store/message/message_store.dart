...
abstract class MessageStoreBase with Store {
  ...
  static ObservableFuture<List<MessageData>> emptyMessResponse =
  ...
  Future<void> removeMessageById({String? id}) async {
    try {
      ...
      if (id != null) {
        ...
        await _requestHelper.removeMessageById(
            id: id, userId: _authStore.user?.id);
      }
      _loadingData = false;
      await refresh();
    } on DioException {
  ...
  Future<void> putRead(MessageData message) async {
    try {
      ...
      List<MessageData> messages = _messages
          .map((MessageData e) => e.id == message.id ? messageRead : e)
          .toList();
