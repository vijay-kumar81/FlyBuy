...
abstract class TransactionStoreStoreBase with Store {
  ...
  // store variables:-----------------------------------------------------------
  static ObservableFuture<List<TransactionWallet>>
      emptyTransactionWalletResponse = ObservableFuture.value([]);

  @observable
  String? _userId;

  @observable
  ObservableFuture<List<TransactionWallet>?> fetchTransactionWalletsFuture =
      emptyTransactionWalletResponse;

  @observable
  ObservableList<TransactionWallet> _transactionWallets =
      ObservableList<TransactionWallet>.of([]);
  ...
  @computed
  bool get loading =>
      fetchTransactionWalletsFuture.status == FutureStatus.pending;
    ...
    if (_userId?.isNotEmpty == true) {
      ...
      return future.then((data) {
        ...
        if (_nextPage <= 1) {
          ...
          _transactionWallets
              .addAll(ObservableList<TransactionWallet>.of(data!));
        }
