...
import 'package:flybuy/store/auth/country_store.dart';
import 'package:flybuy/store/auth/digits_store.dart';
import 'package:flybuy/store/cart/cart_store.dart';
import 'package:flybuy/store/wishlist/wishlist_store.dart';
import 'package:flybuy/store/post_wishlist/post_wishlist_store.dart';
...
abstract class AuthStoreBase with Store {
  ...
  Future<void> loginSuccess(Map<String, dynamic> data) async {
    try {
      ...
      await updateTokenToDatabase(_requestHelper, token,
          topics: user?.roles, userId: user?.id, email: user?.userEmail);
    } catch (e) {
  ...
  Future<bool> editAccount(Map<String, dynamic> data) async {
    ...
    try {
      ...
      userCustomer.addAll({
        'first_name': data['first_name'] is String
            ? data['first_name']
            : _user!.firstName,
        'last_name':
            data['last_name'] is String ? data['last_name'] : _user!.lastName,
        'display_name':
            data['name'] is String ? data['name'] : _user!.displayName,
        'user_email':
            data['email'] is String ? data['email'] : _user!.userEmail,
      });
  ...
  AuthStoreBase(this._persistHelper, this._requestHelper) {
    ...
    wishListStore =
        WishListStore(_persistHelper, _requestHelper, this as AuthStore);
