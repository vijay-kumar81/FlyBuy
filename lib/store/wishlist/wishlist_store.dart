...
abstract class WishListStoreBase with Store {
  ...
  static ObservableFuture<dynamic> emptyWishlistResponse =
    ...
    if (_authStore.isLogin && _enableWishlistPlugin) {
      if (index == -1) {
        ...
        await addWishlistProductShareKey(
            shareKey: shareKey, productId: productId);
      } else {
        _data.removeAt(index);
        int itemId = dataWishlistProduct
  ...
  void _compareDataWishlist(dynamic wishlist) {
    ...
    dynamic dataWishlist =
        _dataWishlistProduct.map((e) => '${e['product_id']}').toList();
    if (data.isEmpty) {
      ...
      if (data.length > dataWishlist.length) {
        ...
        var seen = <String>{};
        dynamic uniqueListId =
            productIds.where((id) => seen.add(id.toString())).toList();
  ...
  Future<dynamic> getWishlistByUser(String id) async {
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        avoidPrint(
            '======== successfully merge product with id=${data[i]} into wishlist ========');
      }
    } on DioException {
      rethrow;
    }
